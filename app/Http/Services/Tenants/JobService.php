...
class JobService implements JobContract
{
    ...
    public function show($id)
    {
        $job = $this
            ->model
            ->with([
                'jobHiringManager:id,first_name,last_name',
                'jobQuestionBank:id,input_type,question',
                'jobRequirement.requirement:id,name,input_type,option',
                'department:id,name',
                'country:id,name',
                'state:id,name',
                'city:id,name'
        ...
        $questionBanks = $this->modelQuestionBank
            ->with(['departments' => function ($query) use ($job) {
                $query->where('department_id', $job->department_id);
            }])
            ->get(['id', 'input_type', 'question']);
